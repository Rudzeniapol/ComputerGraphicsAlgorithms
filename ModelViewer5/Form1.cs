...
namespace ModelViewer5
{
    public partial class Form1 : Form
    {
        private PictureBox renderBox = null!;
        private Button btnLoadObj = null!, btnLoadDiffuse = null!, btnLoadNormal = null!, btnLoadSpecular = null!, btnToggleMode = null!;
        ...
        // БУФЕРЫ ДЛЯ WEIGHTED BLENDED OIT
        private Vector3[] accumColorBuffer = Array.Empty<Vector3>();
        private float[] accumWeightBuffer = Array.Empty<float>();
        private float[] revealageBuffer = Array.Empty<float>();
        ...
        private float objectAlpha = 0.6f; // Коэффициент прозрачности (от 0.0 до 1.0)
        ...
        {
            this.Text = "Model Viewer - Lab 4 + WBOIT Transparency";
            this.Size = new Size(1100, 768);
            this.DoubleBuffered = true;

            int btnHeight = 35;

            btnLoadObj = new Button { Text = "Load OBJ", Location = new Point(10, 10), BackColor = Color.White, Width = 100, Height = btnHeight, TextAlign = ContentAlignment.MiddleCenter };
            btnLoadDiffuse = new Button { Text = "+ Diffuse Map", Location = new Point(120, 10), BackColor = Color.LightGray, Width = 110, Height = btnHeight, TextAlign = ContentAlignment.MiddleCenter };
            btnLoadNormal = new Button { Text = "+ Normal Map", Location = new Point(240, 10), BackColor = Color.LightGray, Width = 110, Height = btnHeight, TextAlign = ContentAlignment.MiddleCenter };
            btnLoadSpecular = new Button { Text = "+ Specular Map", Location = new Point(360, 10), BackColor = Color.LightGray, Width = 110, Height = btnHeight, TextAlign = ContentAlignment.MiddleCenter };
            ...
            btnToggleMode.Click += (s, e) => {
                ...
                btnToggleMode.Text = isTransparentMode ? "Mode: Transparent OIT" : "Mode: Opaque";
                btnToggleMode.BackColor = isTransparentMode ? Color.LightCyan : Color.LightCoral;
            };
            ...
            btnLoadObj.BringToFront(); btnLoadDiffuse.BringToFront(); btnLoadNormal.BringToFront(); btnLoadSpecular.BringToFront(); btnToggleMode.BringToFront();
            CreateRenderTarget();
        }
        ...
        private void CreateRenderTarget()
        {
            if (renderBox.Width > 0 && renderBox.Height > 0) {
                int size = renderBox.Width * renderBox.Height;
                renderTarget?.Dispose();
                renderTarget = new Bitmap(renderBox.Width, renderBox.Height, PixelFormat.Format32bppPArgb);
                zBuffer = new float[size];
                
                // Инициализируем буферы WBOIT под размер экрана
                accumColorBuffer = new Vector3[size];
                accumWeightBuffer = new float[size];
                revealageBuffer = new float[size];
            }
        }
        ...
        private void CalculateVertexNormals()
        {
            ...
            foreach (var face in faces) {
                ...
                for (int i = 0; i < face.Length; i++) normals[face[i].V] = Vector3.Add(normals[face[i].V], faceNormal);
            }
        ...
        private VertexData TransformVertex(FaceVertex fv, Matrix4x4 matWorld, Matrix4x4 transform)
        {
            ...
            return new VertexData {
                ...
                V_over_Z = uv.Y * z_inv,
                Z_inv = z_inv,
                ViewDepth = w // Сохраняем координату W (глубину вида) для расчета весов WBOIT
            };
        }
        ...
        private void Render()
        {
            ...
            unsafe {
                ...
                // Очистка всех буферов экрана
                Parallel.For(0, h, y => {
                    int* row = ptr + (y * w);
                    int offset = y * w;
                    for (int x = 0; x < w; x++) { 
                        row[x] = bg; 
                        zBuffer[offset + x] = float.MaxValue;
                        accumColorBuffer[offset + x] = new Vector3(0, 0, 0);
                        accumWeightBuffer[offset + x] = 0.0f;
                        revealageBuffer[offset + x] = 1.0f; // Коэффициент пропускания света изначально 100%
                    }
                });

                // ВАЖНО: Для OIT накопления необходима атомарность операций сложения/умножения.
                // Чтобы избежать сложных блокировок на CPU, в прозрачном режиме мы пускаем faces в обычном цикле.
                // Благодаря отсутствию сортировки, он отрабатывает мгновенно!
                if (isTransparentMode) {
                    foreach (var face in faces) {
                        ...
                        // Для прозрачности отключаем Backface Culling (мы должны видеть задние стенки изнутри!)
                        for (int i = 1; i < face.Length - 1; i++) {
                            FillTrianglePhongOIT(projFace[0], projFace[i], projFace[i + 1], w, h, matWorld, cameraPos);
                        }
                    }

                    // ПОСТ-ПРОХОД: Композиция Weighted Blended OIT (Финал на весь экран)
                    Parallel.For(0, h, y => {
                        int* row = ptr + (y * w);
                        int offset = y * w;
                        for (int x = 0; x < w; x++) {
                            int idx = offset + x;
                            float r = revealageBuffer[idx];
                            if (r >= 1.0f) continue; // Прозрачных объектов тут не было

                            float accumW = accumWeightBuffer[idx];
                            if (accumW > 0.0f) {
                                // Средний взвешенный цвет прозрачных слоев
                                Vector3 avgColor = Vector3.Multiply(accumColorBuffer[idx], 1.0f / Math.Max(accumW, 0.00001f));
                                
                                // Текущий непрозрачный цвет фона (или геометрии)
                                int currentBg = row[x];
                                float bgR = ((currentBg >> 16) & 0xFF) / 255.0f;
                                float bgG = ((currentBg >> 8) & 0xFF) / 255.0f;
                                float bgB = (currentBg & 0xFF) / 255.0f;

                                // Финальное смешивание по формуле Макгвайра
                                float finalR = avgColor.X * (1.0f - r) + bgR * r;
                                float finalG = avgColor.Y * (1.0f - r) + bgG * r;
                                float finalB = avgColor.Z * (1.0f - r) + bgB * r;

                                int ir = Math.Clamp((int)(finalR * 255f), 0, 255);
                                int ig = Math.Clamp((int)(finalG * 255f), 0, 255);
                                int ib = Math.Clamp((int)(finalB * 255f), 0, 255);

                                row[x] = unchecked((int)0xFF000000 | (ir << 16) | (ig << 8) | ib);
                            }
                        }
                    });

                } else {
                    // ОБЫЧНЫЙ ОРED РЕНДЕРИНГ (Запрограммирован ранее)
                    Parallel.ForEach(faces, face => {
                        ...
                        if (Vector3.Dot(faceNormal, viewDir) >= 0) {
                            for (int i = 1; i < face.Length - 1; i++) {
                                FillTrianglePhongOpaque(projFace[0], projFace[i], projFace[i + 1], ptr, w, h, cameraPos, matWorld);
                            }
                        }
                    });
                }
        ...
        // РАСТЕРИЗАЦИЯ ДЛЯ ПРОЗРАЧНОГО РЕЖИМА (НАКОПЛЕНИЕ WBOIT)
        private unsafe void FillTrianglePhongOIT(VertexData p0, VertexData p1, VertexData p2, int w, int h, Matrix4x4 matWorld, Vector3 cameraPos)
        {
            ...
            for (int y = y_start; y < y_end; y++) {
                ...
                float alphaCoef = (y - p0.ScreenPos.Y) / total_height;
                float betaCoef  = (y - (second_half ? p1.ScreenPos.Y : p0.ScreenPos.Y)) / segment_height;

                VertexData A = VertexData.Lerp(p0, p2, alphaCoef);
                VertexData B = second_half ? VertexData.Lerp(p1, p2, betaCoef) : VertexData.Lerp(p0, p1, betaCoef);
                ...
                for (int x = x_left; x < x_right; x++) {
                    ...
                    // Запись в Z-буфер отключена, но проверка сохраняется 
                    // (чтобы прозрачный объект скрывался за потенциальной непрозрачной стеной)
                    ...
                    int pixelColorInt = CalculatePhongLight(worldP, normalP, u, v, matWorld, cameraPos);
                    
                    // Извлекаем каналы
                    float r = ((pixelColorInt >> 16) & 0xFF) / 255.0f;
                    float g = ((pixelColorInt >> 8) & 0xFF) / 255.0f;
                    float b = (pixelColorInt & 0xFF) / 255.0f;
                    
                    float alpha = objectAlpha; // Используем глобальную полупрозрачность

                    // Интерполируем реальную глубину вида для весовой функции
                    float viewDepth = A.ViewDepth + (B.ViewDepth - A.ViewDepth) * phi;
                    float d = Math.Abs(viewDepth);

                    // СТАБИЛЬНАЯ ВЕСОВАЯ ФУНКЦИЯ МАКГВАЙРА
                    float d5 = d / 5.0f;
                    float d200 = d / 200.0f;
                    float weight = alpha * Math.Clamp(10.0f / (1e-5f + d5 * d5 + d200 * d200 * d200 * d200 * d200 * d200), 0.01f, 3000.0f);

                    // Накапливаем значения в буферы OIT
                    accumColorBuffer[idx].X += r * alpha * weight;
                    accumColorBuffer[idx].Y += g * alpha * weight;
                    accumColorBuffer[idx].Z += b * alpha * weight;
                    
                    accumWeightBuffer[idx] += alpha * weight;
                    revealageBuffer[idx] *= (1.0f - alpha); // Перемножаем прозрачность слоев
                }
            }
        }

        // КЛАССИЧЕСКАЯ НЕПРОЗРАЧНАЯ РАСТЕРИЗАЦИЯ
        private unsafe void FillTrianglePhongOpaque(VertexData p0, VertexData p1, VertexData p2, int* ptr, int w, int h, Vector3 cameraPos, Matrix4x4 matWorld)
        {
            if (p0.ScreenPos.Y > p1.ScreenPos.Y) Swap(ref p0, ref p1);
            if (p0.ScreenPos.Y > p2.ScreenPos.Y) Swap(ref p0, ref p2);
            if (p1.ScreenPos.Y > p2.ScreenPos.Y) Swap(ref p1, ref p2);
        ...
        private int CalculatePhongLight(Vector3 worldPos, Vector3 normal, float u, float v, Matrix4x4 matWorld, Vector3 cameraPos)
        {
            ...
            float specular = 0.0f;
            if (diff > 0.0f) {
                // Изменение направления вектора отражения в соответствии с методичкой: R = L - 2 * (L * N) * N
                Vector3 reflectDir = Vector3.Subtract(lightDir, Vector3.Multiply(normal, 2.0f * Vector3.Dot(lightDir, normal))).Normalize();
    ...
    public struct VertexData {
        ...
        public float U_over_Z, V_over_Z, Z_inv; 
        public float ViewDepth; // Необходима для OIT весов
        ...
        public static VertexData Lerp(VertexData a, VertexData b, float t) {
            return new VertexData {
                ...
                ViewDepth = a.ViewDepth + (b.ViewDepth - a.ViewDepth) * t
            };
        }
    }
    ...
    public struct Vector3 {
        ...
        public static Vector3 Multiply(Vector3 v, float d) => new Vector3(v.X * d, v.Y * d, v.Z * d);
